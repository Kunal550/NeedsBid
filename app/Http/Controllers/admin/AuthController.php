...
class AuthController extends Controller
{
            ...
            case $request->isMethod('POST'):
                ...
                if($validator->fails()){
                    ...
                    if(Auth::validate(['email' => $request->email, 'password' => $request->password])){
                        // if(Auth::attempt(['email' => $request->email, 'password' => $request->password,'user_type_id' => 1], $remember)){
                            if(Auth::attempt(['email' => $request->email, 'password' => $request->password], $remember)){
                            Auth::logoutOtherDevices($request->password);
                            return redirect()->route('admin.dashboard')->withSuccess('Logged in Successfully.');
                        }else if(Auth::attempt(['email' => $request->email, 'password' => $request->password,'user_type_id' =>3], $remember)){
                            Auth::logoutOtherDevices($request->password);
                            return redirect()->route('admin.dashboard')->withSuccess('Logged in Successfully.');
                        }else{
    ...
    public function logout(Request $request){
        ...
        return redirect()->route('admin.login')->withSuccess('You are now logged out of the system.');
    }
}